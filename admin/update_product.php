...
if (isset($_POST['update'])) {
   ...
   $update_product = $conn->prepare("UPDATE `products` SET name = ?, price = ?, description = ?,details = ? WHERE id = ?");
   $update_product->execute([$name, $price, $description, $details, $pid]);
      ...
      if ($select_products->rowCount() > 0) {
         while ($fetch_products = $select_products->fetch(PDO::FETCH_ASSOC)) {
            ...
            <form action="" method="post" enctype="multipart/form-data">
               ...
               <input type="text" name="price" required class="box" min="0" max="9999999999" placeholder="enter product price"
                  onkeypress="if(this.value.length == 10) return false;" value="<?= $fetch_products['price']; ?>">
               <span>update description</span>
               <textarea name="description" class="box" required cols="30"
                  rows="10"><?= $fetch_products['description']; ?></textarea>
